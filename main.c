...
void *addCounter(void *arg) {
    ...
    while (1) {
        ...
        sem_wait(&counter_sem);//ensuring that no two threads from mCounter or mMonitor are modifying in the counter at the same time
...
int main() {
    ...
    sem_init(&full_sem, 0, 0);//Initialzing full_sem with zerthe buffer: Monitor threado which means that we can take up to SIZE items from the buffer
